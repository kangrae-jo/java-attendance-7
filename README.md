...
| Crew         | 우테코에 등교하는 수강생 |
| Crew Name    | 크루 이름         |
| Arrival Time | 크루가 등교한 시간    |

### 학교

| 용어        | 설명                      |
|-----------|-------------------------|
| School    | 크루들이 등교해야하는 곳, 즉 우테코 건물 |
| StartTime | 교육이 시작하는 시간, 즉 등교시간     |
| EndTime   | 교육이 끝나는 시간, 즉 하교시간      |
...
|---------|------------------------------------------|
| Late    | 지각 (등교시간 5분 초과)                          |
| Absence | 결석 (등교시간 30분 초과 or 출석 x, 지각 3회 == 결석 1회) |
| Waring  | 경고 (결석 2회 이상)                            |
| Consul  | 상담 (결석 3회 이상)                            |
| Expel   | 제적 (결석 5회 초과)                            |
...
- [ ] 시간은 24시간 형식만 사용한다. 예를 들어, "22:30은 오후 10시 30분을 의미한다.
- [ ] 교육 시간은 월요일은 13:00~18:00, 화요일~ 금요일은 10:00~18:00이다.
    ...
    - [ ] 누적 지각 및 결석 횟수에 따라 경고 또는 면담을 시행한다. 또한 결석 횟수가 5회를 초 과할 때 제적을 시행한다.
    - [ ] 지각 3회는 결석 1회로 간주한다.
    - [ ] 경고 대상자: 결석 2회 이상
    - [ ] 면담 대상자: 결석 3회 이상
    - [ ] 제적 대상자: 결석 5회 초과
...
- [ ] 사용자가 잘못된 값을 입력할 경우 "[ERROR]"로 시작하는 메시지와 함 께 IllegalArgumentException 을 발생시킨 후 애플리케이션은 종료되어야 한다.
...
- [ ] 제적 위험자는 제적 대상자, 면담 대상자, 경고 대상자순으로 출력하며, 대상 항목별 정 렬 순서는 지각을 결석으로 간주하여 내림차순한다.
