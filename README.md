...
|-----------|----------------------------------------------|
| State     | 지각 또는 결석                                     |
| Tardiness | 지각 (등교시간보다 5분 초과)                            |
| Absence   | 결석 (등교시간보다 30분 초과 or 안왔음) + (지각 3회 == 결석 1회) |
...
| Policy    | 결석 횟수에 따른 정책  |
| Warning   | 경고 (결석 2회 이상) |
| Interview | 면담 (결석 3회 이상) |
| Expulsion | 제적 (결석 5회 초과) |
...
- [x] 시간은 24시간 형식만 사용한다. 예를 들어, "22:30은 오후 10시 30분을 의미한다.
- [x] 교육 시간은 월요일은 13:00~18:00, 화요일~ 금요일은 10:00~18:00이다.
    ...
    - [x] 누적 지각 및 결석 횟수에 따라 경고 또는 면담을 시행한다. 또한 결석 횟수가 5회를 초 과할 때 제적을 시행한다.
...
- [x] 사용자가 잘못된 값을 입력할 경우 "[ERROR]"로 시작하는 메시지와 함께 IllegalArgumentException 을 발생시킨 후 애플리케이션은 종료되어야 한다.
